...
`continuous`, `bounded`, `nominal`, and `ordinal` are implemented; every other
support raises NotImplementedError so a half-validated value space can never be
stored. Each deferred branch is a one-function addition when its first real use
arrives.
"""
...
def validate_value(support: Support, value: object) -> None:
    ...
    if t == SupportType.bounded:
        # Same numeric rule as continuous, plus inclusive [lo, hi] range.
        # bounds are guaranteed present and lo < hi by the Support schema.
        if isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError(f"bounded support requires a real number; got {value!r}")
        lo, hi = support.bounds
        if not lo <= value <= hi:
            raise ValueError(f"bounded support value {value} is outside [{lo}, {hi}]")
        return
    ...
    raise NotImplementedError(
        f"support '{t.value}' is not implemented yet "
        "(continuous, bounded, nominal, ordinal)"
    )
